...
def run_tests() -> bool:
    ...
    # ----------------------------------------------------
    # TEST SUITE 1: Timing Boundary Tests
    # ----------------------------------------------------
    print("\n--- TEST SUITE 1: Timing Boundary ---")
    sim = RelaysSimulator()
    sim.begin()

    # 1.1: Request state change at t=1000
    t0 = 1000
    ...
    # 1.2: Request opposite state during cooldown at t=1500
    ...
    # 1.3: Tick at 999ms elapsed (t=1999)
    sim.tick(1999)
    ...
    # 1.4: Tick at exactly 1000ms elapsed (t=2000)
    sim.tick(2000)
    ...
    # 1.5: Cooldown timer restarted after applying pending state
    ...
    # Try another command immediately at t=2050
    ...
    # 1.6: Idempotent queueing
    sim.request_state_change(0, False, 3100) # queue OFF
    sim.request_state_change(0, True, 3200)  # overwrite with ON (same as current)
    ...
    # Rollover to 0 (now becomes 1 in tick: 502ms elapsed)
    sim.tick(0)
    assert sim.get_state(1) == True
    assert sim.has_pending(1) == True

    # At 999ms elapsed from start_t:
    # elapsed = now - (0xFFFFFFFF - 500) = now + 501
    # 999 = now + 501 => now = 498
    sim.tick(498)
    assert sim.get_state(1) == True, "FAIL 2.1: premature apply across rollover at 999ms elapsed"
    assert sim.has_pending(1) == True

    # At 1000ms elapsed: now = 499
    sim.tick(499)
    ...
    # 2.2: Cooldown starts exactly at 0xFFFFFFFF
    sim.begin()
    sim.request_state_change(2, True, 0xFFFFFFFF)
    sim.request_state_change(2, False, 0xFFFFFFFF) # queued
    assert sim.has_pending(2) == True
    # 999ms elapsed: now = 998
    sim.tick(998)
    assert sim.get_state(2) == True
    assert sim.has_pending(2) == True
    # 1000ms elapsed: now = 999
    sim.tick(999)
    ...
    # 2.3: Cooldown starts at millis() == 0 (sentinel avoidance now=1)
    sim.begin()
    ...
    sim.tick(1000) # elapsed = 1000 - 1 = 999ms
    assert sim.get_state(3) == True
    assert sim.has_pending(3) == True
    sim.tick(1001) # elapsed = 1001 - 1 = 1000ms
    ...
    # 2.4: Exhaustive boundary sweep around 0xFFFFFFFF
    sim.begin()
    ...
    # 3.2: Empty lines and whitespace
    harness.output_lines.clear()
    ...
    # 3.3: Multiple spaces and tabs in valid commands
    whitespace_valid = [
    ...
    # 3.4: Oversized serial commands (>64 characters)
    oversized_cases = [
        "A" * 65,
        "RELAY 1 ON " + ("x" * 60),
        "RELAY " + ("9" * 70) + " ON",
    ...
    for cmd in oversized_cases:
        harness.output_lines.clear()
        harness.feed_bytes(f"{cmd}\n".encode('latin1'))
        # Should not crash, should produce either ERR UNKNOWN CMD or valid trimmed command
    ...
    # 3.5: Split chunk feed (simulating slow UART stream)
    harness.output_lines.clear()
    ...
    # ----------------------------------------------------
    # TEST SUITE 4: Rapid Burst Stress Tests
    # ----------------------------------------------------
    ...
    # 4.1: 100 rapid commands sent within 10ms across all 4 channels
    burst_commands = []
    for i in range(100):
        ch = (i % RELAY_COUNT) + 1
    ...
    # Feed all 100 commands at t=1000..1010
    start_time = 1000
    ...
    # Check physical actuations so far: each channel must have actuated AT MOST ONCE!
    ...
    # 4.2: Fuzzing stress test: 10,000 randomized commands over simulated time
    ...
    # Invariant verification across all history
    for ch_idx in range(RELAY_COUNT):
